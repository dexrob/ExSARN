...
def main(args):
    ...
    train_loader, val_loader, test_loader = get_TrainValTestLoader(dataset_dir, k=kfold_number, batch_size=batch_size,shuffle=shuffle, prefix=prefix,seq_len=sequence_length, p=sample_percent, i=i)    # X_train, X_val, X_test, Y_train, Y_val, Y_test = get_TrainValTestData(dataset_dir, k=kfold_number, prefix=prefix,seq_len=sequence_length)
    ...
    mid_norm = None # "BN"
    model_name = "BT19_{}_ConcatPathway_stiffness{}_roughness{}_p_thre_{}_rs_{}_ll{}_ihs{}_ohs{}_rf{}_res{}_rec{}_mid{}o{}_ep{}_lr{}_wpr_{}_wps_{}_wc{}_wr1{}_wr2{}_wkl{}_var{}_rep{}_k{}".format(str(use_stiffness), str(use_roughness), str(p_threshold), str(args.rand_seed), data_folder[:-1], latent_length, receptor_hidden_size, mid_hidden_size, str(receptor_freq), residual, receptor_block, mid_block, mid_lmu_order, n_epochs, learning_rate, w_p_r, w_p_s, w_c, w_r1, w_r2, w_kl, var, rep, str(kfold_number))

    model = ConcatPathway_property_layer(number_of_features=number_of_features,
                        num_class=num_class,
                        x_freq=x_freq,
                        header=header,
    ...
    for epoch in range(n_epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            ...
            if i % data_reduction_ratio == 0:
                train_num += x.size(0)
                optimizer.zero_grad()
                inner_h_n, dec_r_out, enc_h_n, dec_input, latent, output, s, r = model(x)
    ...
    dict_name = model_name + '_fold{}_{}.pkl'.format(str(kfold_number), rep)
    pickle.dump(results_dict, open(logDir + dict_name, 'wb'))
    print("dump results dict to {}".format(dict_name))
    return test_acc, test_stiffness_acc, test_roughness_acc
...
parser.add_argument("--w_p_r", default=5.0, type=float, help="weight of property classification loss")
parser.add_argument("--w_p_s", default=5.0, type=float, help="weight of property classification loss")
...
parser.add_argument("--sample_percent", default=0.6, type=int, help="percentage of samples for training")
...
for (r, s, w_p_r, w_p_s) in [(0,0,0,0),(1,1,0.2,0.2),(1,1,0.2,0.5),(1,1,0.5,0.5)]:
    args.use_stiffness = s
    args.use_roughness = r
    args.w_p_r = w_p_r
    args.w_p_s = w_p_s
    for p in [0.3, 0.2]:
...
# for data_folder in ["c20comb/", "c50/", "c30/"]:
