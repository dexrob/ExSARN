...
def main(args):
    ...
    model_dir = 'models_and_stats/BT19_1_ConcatPathway_stiffness1_roughness0.25_p_thre_1_rs_c20_ll40_ihs[100, 200]_ohs90_rf[20, 10]_res1_recGRU_midLSTMo3_ep100_lr0.001_wp_2_wc1_wr10.001_wr20.001_wkl0.0005_var1_rep0_k099.pt'
    saved_dicts = torch.load(model_dir, map_location='cpu')
    pretrained_dict = saved_dicts['model_state_dict']
    pretrained_dict = {k: v for k, v in pretrained_dict.items() if 'eheader' not in k and 'receptors' not in k and 'classifier' not in k}
    model.load_state_dict(pretrained_dict, strict=False)
    
    for name, param in model.named_parameters():
        if 'eheader' not in name and 'receptors' not in name and 'classifier' not in name:
