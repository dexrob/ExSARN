...
def train(args):
    ...
    model_name = 'p_in_lat_stiffness{}_roughness{}_p_thre_{}_rs_{}_B_block_{}_data_{}_wrI_{}_wC_{}_wps_{}_wpr_{}_hidden_{}_latent_{}_k_{}_p_{}'.format(str(use_stiffness), str(use_roughness), str(p_threshold), str(args.rand_seed), block, dataset, w_r, w_c, w_p_s, w_p_r, str(hidden_size), str(latent_length), str(kfold_number), str(sample_percent))
...
for (r, s, w_p_r, w_p_s) in [(0,0,0,0),(1,1,0.2,0.2),(1,1,0.2,0.5),(1,1,0.2,1)]:
    ...
    for p in [0.3, 0.2]:
        ...
        for i in i_: 
            ...
            for k in range(5):
                args.kfold = k
                print(args)
                test_acc, test_stiffness_acc, test_roughness_acc = train(args)
